...
public class FatherNewFamilyToNewInsuranceClient__FWD_69_localSearch extends GenericLocalSearchActor{
	...
	protected void initializePorts(Map<String, ActorRef> name2actor) {
		...
		ports.add(new PortJunction(node.getPorts().getPort().get(0), getSelf(), name2actor.get("FatherNewFamilyToNewInsuranceClient__CONSISTENCY_72_localSearch"), this::returnTrue  , false ));
		ports.add(new PortJunction(node.getPorts().getPort().get(1), getSelf(), name2actor.get("FatherNewFamilyToNewInsuranceClient__FWD_production"), this::returnTrue  , false ));
	}
			...
			case "FamilyRegisterToInsuranceDatabase_object_SP1": 
				{
					...
					HMatch match_1 = new LocalSearchMatch("FatherNewFamilyToNewInsuranceClient__FWD_69_localSearch", 5);
			...
			case "InsuranceDatabase_object_SP2": 
				{
					if(!lazy_initialization) {
						...
						// insuranceDB
						HMatch match_2 = new LocalSearchMatch("FatherNewFamilyToNewInsuranceClient__FWD_69_localSearch", 5);
						match_2.getNodes()[2] = objs[0];
						start(edge_explorer_3_1_orchestration, match_2);
					}
				}
				break;
			case "FatherNewFamilyToNewInsuranceClient__SRC_65_localSearch": 
				{
					if(pac_explorer.register(match)) {
						HMatch acMatch_0 = new LocalSearchMatch("FatherNewFamilyToNewInsuranceClient__FWD_69_localSearch", 5);
		...
		case "Families2Insurance.FamilyRegisterToInsuranceDatabase_source_FamilyRegister": 
			if(!lazy_initialization) {
				if(msg.target instanceof edu.kit.ipd.sdq.metamodels.families.FamilyRegister) {
				{
					...
					HMatch match = new LocalSearchMatch("FatherNewFamilyToNewInsuranceClient__FWD_69_localSearch", 5);
					Object[] objs = match.getNodes();
					objs[1] = msg.source;
					objs[0] = msg.target;
					currentDepth++;
	...
	protected void registerMatch(HMatch match) {
		...
		Object[] objs = match.getNodes();
		// register FatherNewFamilyToNewInsuranceClient__SRC
	...
	protected void deregisterMatch(HMatch match) {
		...
		Object[] objs = match.getNodes();
		// deregister FatherNewFamilyToNewInsuranceClient__SRC
		OverlapMatch oMatch_0 = new OverlapMatch(3);
		Object[] oMatch_Nodes_0 = oMatch_0.getNodes();
		oMatch_Nodes_0[0] = objs[0];
		oMatch_Nodes_0[1] = objs[3];
