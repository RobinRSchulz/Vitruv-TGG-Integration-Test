...
public class MotherToInsuranceClient__FWD_170_localSearch extends GenericLocalSearchActor{
	...
	protected void initializePorts(Map<String, ActorRef> name2actor) {
		...
		ports.add(new PortJunction(node.getPorts().getPort().get(0), getSelf(), name2actor.get("MotherToInsuranceClient__CONSISTENCY_173_localSearch"), this::returnTrue  , false ));
		ports.add(new PortJunction(node.getPorts().getPort().get(1), getSelf(), name2actor.get("MotherToInsuranceClient__FWD_production"), this::returnTrue  , false ));
	}
			...
			case "FamilyRegisterToInsuranceDatabase_object_SP0": 
				{
					...
					HMatch match_2 = new LocalSearchMatch("MotherToInsuranceClient__FWD_170_localSearch", 5);
			...
			case "InsuranceDatabase_object_SP5": 
				{
					if(!lazy_initialization) {
						...
						// insuranceDB
						HMatch match_3 = new LocalSearchMatch("MotherToInsuranceClient__FWD_170_localSearch", 5);
						match_3.getNodes()[3] = objs[0];
						start(edge_explorer_3_1_orchestration, match_3);
					}
				}
				break;
			case "MotherToInsuranceClient__SRC_166_localSearch": 
				{
					if(pac_explorer.register(match)) {
						HMatch acMatch_0 = new LocalSearchMatch("MotherToInsuranceClient__FWD_170_localSearch", 5);
		...
		case "Something2Else.FamilyRegisterToInsuranceDatabase_source_FamilyRegister": 
			if(!lazy_initialization) {
				if(msg.target instanceof edu.kit.ipd.sdq.metamodels.families.FamilyRegister) {
				{
					...
					HMatch match = new LocalSearchMatch("MotherToInsuranceClient__FWD_170_localSearch", 5);
					Object[] objs = match.getNodes();
					objs[2] = msg.source;
					objs[1] = msg.target;
					currentDepth++;
	...
	protected void registerMatch(HMatch match) {
		...
		Object[] objs = match.getNodes();
		// register MotherToInsuranceClient__SRC
	...
	protected void deregisterMatch(HMatch match) {
		...
		Object[] objs = match.getNodes();
		// deregister MotherToInsuranceClient__SRC
		OverlapMatch oMatch_0 = new OverlapMatch(3);
		Object[] oMatch_Nodes_0 = oMatch_0.getNodes();
		oMatch_Nodes_0[0] = objs[0];
		oMatch_Nodes_0[1] = objs[1];
