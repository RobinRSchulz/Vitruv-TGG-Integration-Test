...
public class DaughterToInsuranceClient__FWD_20_localSearch extends GenericLocalSearchActor{
	...
	protected void initializePorts(Map<String, ActorRef> name2actor) {
		...
		ports.add(new PortJunction(node.getPorts().getPort().get(0), getSelf(), name2actor.get("DaughterToInsuranceClient__FWD_production"), this::returnTrue  , false ));
	}
			...
			case "Family_object_SP4": 
				{
					...
					// family
					HMatch match_0 = new LocalSearchMatch("DaughterToInsuranceClient__FWD_20_localSearch", 5);
			...
			case "FamilyRegister_object_SP1": 
				{
					if(!lazy_initialization) {
						...
						// familyRegister
						HMatch match_1 = new LocalSearchMatch("DaughterToInsuranceClient__FWD_20_localSearch", 5);
			...
			case "FamilyRegisterToInsuranceDatabase_object_SP1": 
				{
					...
					HMatch match_2 = new LocalSearchMatch("DaughterToInsuranceClient__FWD_20_localSearch", 5);
			...
			case "InsuranceDatabase_object_SP1": 
				{
					if(!lazy_initialization) {
						...
						// insuranceDB
						HMatch match_3 = new LocalSearchMatch("DaughterToInsuranceClient__FWD_20_localSearch", 5);
			...
			case "Member_object_SP4": 
				{
					if(!lazy_initialization) {
						...
						// daughter
						HMatch match_4 = new LocalSearchMatch("DaughterToInsuranceClient__FWD_20_localSearch", 5);
		...
		switch(match.creator) {
			...
			case "DaughterToInsuranceClient_daughter_sons_incoming_SRC__FILTER_NAC_SRC_1_localSearch": 
				if(nac_checker.deregister(match)) {
					HMatch acMatch_0 = new LocalSearchMatch("DaughterToInsuranceClient__FWD_20_localSearch", 5);
					Object[] acMatch_nodes_0 = acMatch_0.getNodes();
			...
			case "DaughterToInsuranceClient_daughter_father_incoming_SRC__FILTER_NAC_SRC_4_localSearch": 
				if(nac_checker_0.deregister(match)) {
					HMatch acMatch_1 = new LocalSearchMatch("DaughterToInsuranceClient__FWD_20_localSearch", 5);
					Object[] acMatch_nodes_1 = acMatch_1.getNodes();
			...
			case "DaughterToInsuranceClient_daughter_mother_incoming_SRC__FILTER_NAC_SRC_7_localSearch": 
				if(nac_checker_1.deregister(match)) {
					HMatch acMatch_2 = new LocalSearchMatch("DaughterToInsuranceClient__FWD_20_localSearch", 5);
					Object[] acMatch_nodes_2 = acMatch_2.getNodes();
		...
		case "Families2Insurance.FamilyRegisterToInsuranceDatabase_source_FamilyRegister": 
			if(!lazy_initialization) {
				if(msg.target instanceof edu.kit.ipd.sdq.metamodels.families.FamilyRegister) {
				{
					...
					HMatch match = new LocalSearchMatch("DaughterToInsuranceClient__FWD_20_localSearch", 5);
					Object[] objs = match.getNodes();
					objs[2] = msg.source;
					objs[1] = msg.target;
					currentDepth++;
