...
public class Staff_object_SP0 extends GenericObjectActor<AdministrationExample.Staff> {
	...
	protected void initializePorts(Map<String, ActorRef> name2actor, ObjectNode node) {
		...
		ports.add(new PortNodeRight<AdministrationExample.Staff>(node.getPorts().getPort().get(0), getSelf(), name2actor.get("Administration_staff_0_reference"), this::check_constraint_3 , 0   ));
		ports.add(new PortNodeRight<AdministrationExample.Staff>(node.getPorts().getPort().get(3), getSelf(), name2actor.get("Administration_staff_1_reference"), this::check_constraint_15 , 1   ));
		ports.add(new PortNodeMatch<AdministrationExample.Staff>(node.getPorts().getPort().get(1), getSelf(), name2actor.get("DoctorShiftplanRule__BWD_43_localSearch"), this::returnTrue   , node.getName() ));
		ports.add(new PortNodeMatch<AdministrationExample.Staff>(node.getPorts().getPort().get(2), getSelf(), name2actor.get("DoctorToPatient__CONSISTENCY_76_localSearch"), this::returnTrue   , node.getName() ));
	}
	...
	public boolean check_constraint_15(AdministrationExample.Staff s2, int index) {
