package Hospital2AdministrationSolutions.initbwd.hipe.engine.actor.junction;
...
public class NurseToStaffRule__BWD_137_junction extends GenericJunctionActor{
	private Map<Object, Collection<HMatch>> staff_13_AttrMap = HiPEMultiUtil.createMap();
	private Map<Object, Collection<HMatch>> department_13_AttrMap = HiPEMultiUtil.createMap();
	
	@Override
	protected void initializePorts(Map<String, ActorRef> name2actor, JunctionNode node) {
		constraints.add(this::check_constraint_18);
	...
	protected void registerMatchForAttributeChanges(HMatch match) {
		...
		Collection<HMatch> staff_6_Matches = staff_13_AttrMap.get(matchObjects[6]);
		if(staff_6_Matches == null) {
			staff_6_Matches = new LinkedList<>();
			staff_13_AttrMap.put(matchObjects[6], staff_6_Matches);
		}
		
		staff_6_Matches.add(match);
		
		Collection<HMatch> department_0_Matches = department_13_AttrMap.get(matchObjects[0]);
		if(department_0_Matches == null) {
			department_0_Matches = new LinkedList<>();
			department_13_AttrMap.put(matchObjects[0], department_0_Matches);
		}
	...
	protected void deregisterMatchForAttributeChanges(Set<HMatch> matches, HMatch match) {
		Object[] matchObjects = match.getNodes();
		Collection<HMatch> matches_0 = staff_13_AttrMap.get(matchObjects[6]);
		if(matches_0 != null) {
			matches_0.remove(match);
		}
		Collection<HMatch> matches_1 = department_13_AttrMap.get(matchObjects[0]);
		if(matches_1 != null) {
	...
	protected void changeAttribute(AttributeChanged<HMatch> message) {
		...
		if(obj instanceof AdministrationExample.Staff) {
			if(staff_13_AttrMap.containsKey(obj)) {
				for(HMatch attr_match : staff_13_AttrMap.get(obj)) {
		...
		if(obj instanceof HospitalExample.Department) {
			if(department_13_AttrMap.containsKey(obj)) {
				for(HMatch attr_match : department_13_AttrMap.get(obj)) {
					for(int i=0; i<ports.size(); i++) {
						Port<HMatch> port = ports.get(i);
						HMatch match = attr_match;
