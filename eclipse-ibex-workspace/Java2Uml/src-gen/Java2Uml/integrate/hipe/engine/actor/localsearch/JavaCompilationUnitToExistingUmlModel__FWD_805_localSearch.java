...
public class JavaCompilationUnitToExistingUmlModel__FWD_805_localSearch extends GenericLocalSearchActor{
	...
	protected void initializePorts(Map<String, ActorRef> name2actor) {
		...
		ports.add(new PortJunction(node.getPorts().getPort().get(0), getSelf(), name2actor.get("JavaCompilationUnitToExistingUmlModel__CONSISTENCY_808_localSearch"), this::returnTrue  , false ));
		ports.add(new PortJunction(node.getPorts().getPort().get(1), getSelf(), name2actor.get("JavaCompilationUnitToExistingUmlModel__FWD_production"), this::returnTrue , 0  , false ));
	}
			...
			case "JavaCompilationUnitToUmlModel_object": 
				{
					...
					HMatch match_1 = new LocalSearchMatch("JavaCompilationUnitToExistingUmlModel__FWD_805_localSearch", 5);
			...
			case "Model_object_SP1": 
				{
					if(!lazy_initialization) {
						...
						// umlModel
						HMatch match_3 = new LocalSearchMatch("JavaCompilationUnitToExistingUmlModel__FWD_805_localSearch", 5);
						match_3.getNodes()[3] = objs[0];
						start(edge_explorer_3_1_orchestration, match_3);
					}
				}
				break;
			case "JavaCompilationUnitToExistingUmlModel__SRC_801_localSearch": 
				{
					if(pac_explorer.register(match)) {
						HMatch acMatch_0 = new LocalSearchMatch("JavaCompilationUnitToExistingUmlModel__FWD_805_localSearch", 5);
		...
		case "Java2Uml.JavaCompilationUnitToUmlModel_source_CompilationUnit": 
			if(!lazy_initialization) {
				if(msg.target instanceof org.emftext.language.java.containers.CompilationUnit) {
				{
					...
					HMatch match = new LocalSearchMatch("JavaCompilationUnitToExistingUmlModel__FWD_805_localSearch", 5);
					Object[] objs = match.getNodes();
					objs[1] = msg.source;
					objs[0] = msg.target;
					currentDepth++;
	...
	protected void registerMatch(HMatch match) {
		...
		Object[] objs = match.getNodes();
		// register JavaCompilationUnitToExistingUmlModel__SRC
	...
	protected void deregisterMatch(HMatch match) {
		...
		Object[] objs = match.getNodes();
		// deregister JavaCompilationUnitToExistingUmlModel__SRC
		OverlapMatch oMatch_0 = new OverlapMatch(3);
		Object[] oMatch_Nodes_0 = oMatch_0.getNodes();
		oMatch_Nodes_0[0] = objs[0];
		oMatch_Nodes_0[1] = objs[2];
	...
	public boolean constraint_checker_method(HMatch match) {
		...
		csp_94.getVariables().add(new org.emoflon.ibex.tgg.operational.csp.RuntimeTGGAttributeConstraintVariable(true, "UML_MODEL", "java.lang.String"));
		csp_94.getVariables().add(new org.emoflon.ibex.tgg.operational.csp.RuntimeTGGAttributeConstraintVariable(true, umlModel.getName(), "java.lang.String"));
		csp_94.solve();
