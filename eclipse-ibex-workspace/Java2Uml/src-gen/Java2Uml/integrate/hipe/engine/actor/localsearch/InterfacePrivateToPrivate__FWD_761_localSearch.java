...
public class InterfacePrivateToPrivate__FWD_761_localSearch extends GenericLocalSearchActor{
	...
	protected void initializePorts(Map<String, ActorRef> name2actor) {
		...
		ports.add(new PortJunction(node.getPorts().getPort().get(0), getSelf(), name2actor.get("InterfacePrivateToPrivate__CONSISTENCY_764_localSearch"), this::returnTrue  , false ));
		ports.add(new PortJunction(node.getPorts().getPort().get(1), getSelf(), name2actor.get("InterfacePrivateToPrivate__FWD_production"), this::returnTrue , 0  , false ));
	}
			...
			case "JavaClassifierToUmlClassifier_object_SP23": 
				{
					...
					HMatch match_0 = new LocalSearchMatch("InterfacePrivateToPrivate__FWD_761_localSearch", 4);
			...
			case "Interface_object_SP6": 
				{
					if(!lazy_initialization) {
						...
						// umlClassifier
						HMatch match_2 = new LocalSearchMatch("InterfacePrivateToPrivate__FWD_761_localSearch", 4);
						match_2.getNodes()[2] = objs[0];
						start(edge_explorer_3_1_orchestration, match_2);
					}
				}
				break;
			case "InterfacePrivateToPrivate__SRC_758_localSearch": 
				{
					if(pac_explorer.register(match)) {
						HMatch acMatch_0 = new LocalSearchMatch("InterfacePrivateToPrivate__FWD_761_localSearch", 4);
		...
		case "Java2Uml.JavaClassifierToUmlClassifier_source_Classifier": 
			if(!lazy_initialization) {
				if(msg.target instanceof org.emftext.language.java.classifiers.Interface) {
				{
					...
					HMatch match = new LocalSearchMatch("InterfacePrivateToPrivate__FWD_761_localSearch", 4);
					Object[] objs = match.getNodes();
					objs[0] = msg.source;
					objs[1] = msg.target;
					currentDepth++;
	...
	protected void registerMatch(HMatch match) {
		...
		Object[] objs = match.getNodes();
		// register InterfacePrivateToPrivate__SRC
	...
	protected void deregisterMatch(HMatch match) {
		...
		Object[] objs = match.getNodes();
		// deregister InterfacePrivateToPrivate__SRC
		OverlapMatch oMatch_0 = new OverlapMatch(2);
		Object[] oMatch_Nodes_0 = oMatch_0.getNodes();
		oMatch_Nodes_0[0] = objs[1];
		oMatch_Nodes_0[1] = objs[3];
	...
	public boolean constraint_checker_method(HMatch match) {
		...
		org.emoflon.ibex.tgg.operational.csp.constraints.custom.java2uml.UserDefined_expectPrivate csp_85 = new org.emoflon.ibex.tgg.operational.csp.constraints.custom.java2uml.UserDefined_expectPrivate();
		csp_85.getVariables().add(new org.emoflon.ibex.tgg.operational.csp.RuntimeTGGAttributeConstraintVariable(true, umlClassifier.getVisibility(), "Enum::VisibilityKind"));
		csp_85.solve();
