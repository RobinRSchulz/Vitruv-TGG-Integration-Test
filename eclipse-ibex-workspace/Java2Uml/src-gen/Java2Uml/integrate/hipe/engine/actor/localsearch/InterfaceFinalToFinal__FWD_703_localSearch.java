...
public class InterfaceFinalToFinal__FWD_703_localSearch extends GenericLocalSearchActor{
	...
	protected void initializePorts(Map<String, ActorRef> name2actor) {
		...
		ports.add(new PortJunction(node.getPorts().getPort().get(0), getSelf(), name2actor.get("InterfaceFinalToFinal__CONSISTENCY_706_localSearch"), this::returnTrue  , false ));
		ports.add(new PortJunction(node.getPorts().getPort().get(1), getSelf(), name2actor.get("InterfaceFinalToFinal__FWD_production"), this::returnTrue , 0  , false ));
	}
			...
			case "JavaClassifierToUmlClassifier_object_SP17": 
				{
					...
					HMatch match_0 = new LocalSearchMatch("InterfaceFinalToFinal__FWD_703_localSearch", 4);
			...
			case "Interface_object_SP1": 
				{
					if(!lazy_initialization) {
						...
						// umlClassifier
						HMatch match_2 = new LocalSearchMatch("InterfaceFinalToFinal__FWD_703_localSearch", 4);
						match_2.getNodes()[2] = objs[0];
						start(edge_explorer_3_1_orchestration, match_2);
					}
				}
				break;
			case "InterfaceFinalToFinal__SRC_700_localSearch": 
				{
					if(pac_explorer.register(match)) {
						HMatch acMatch_0 = new LocalSearchMatch("InterfaceFinalToFinal__FWD_703_localSearch", 4);
		...
		case "Java2Uml.JavaClassifierToUmlClassifier_source_Classifier": 
			if(!lazy_initialization) {
				if(msg.target instanceof org.emftext.language.java.classifiers.Interface) {
				{
					...
					HMatch match = new LocalSearchMatch("InterfaceFinalToFinal__FWD_703_localSearch", 4);
					Object[] objs = match.getNodes();
					objs[0] = msg.source;
					objs[1] = msg.target;
					currentDepth++;
	...
	protected void registerMatch(HMatch match) {
		...
		Object[] objs = match.getNodes();
		// register InterfaceFinalToFinal__SRC
	...
	protected void deregisterMatch(HMatch match) {
		...
		Object[] objs = match.getNodes();
		// deregister InterfaceFinalToFinal__SRC
		OverlapMatch oMatch_0 = new OverlapMatch(2);
		Object[] oMatch_Nodes_0 = oMatch_0.getNodes();
		oMatch_Nodes_0[0] = objs[1];
		oMatch_Nodes_0[1] = objs[3];
	...
	public boolean constraint_checker_method(HMatch match) {
		...
		csp_78.getVariables().add(new org.emoflon.ibex.tgg.operational.csp.RuntimeTGGAttributeConstraintVariable(true, umlClassifier.isIsFinalSpecialization(), "boolean"));
		csp_78.solve();
