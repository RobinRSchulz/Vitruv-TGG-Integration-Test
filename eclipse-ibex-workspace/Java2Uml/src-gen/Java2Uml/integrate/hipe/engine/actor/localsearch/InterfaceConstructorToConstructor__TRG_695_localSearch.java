...
public class InterfaceConstructorToConstructor__TRG_695_localSearch extends GenericLocalSearchActor{
	...
	protected void initializePorts(Map<String, ActorRef> name2actor) {
		...
		ports.add(new PortJunction(node.getPorts().getPort().get(0), getSelf(), name2actor.get("InterfaceConstructorToConstructor__BWD_698_localSearch"), this::returnTrue  , false ));
		ports.add(new PortJunction(node.getPorts().getPort().get(1), getSelf(), name2actor.get("InterfaceConstructorToConstructor__TRG_production"), this::returnTrue  , false ));
	}
			...
			case "Interface_object_SP9": 
				{
					...
					// umlClassifier
					HMatch match_0 = new LocalSearchMatch("InterfaceConstructorToConstructor__TRG_695_localSearch", 2);
			...
			case "Operation_object_SP0": 
				{
					if(!lazy_initialization) {
						...
						// umlOperation
						HMatch match_1 = new LocalSearchMatch("InterfaceConstructorToConstructor__TRG_695_localSearch", 2);
		...
		switch(match.creator) {
			...
			case "InterfaceConstructorToConstructor_umlOperation_ownedOperation_incoming_TRG__FILTER_NAC_TRG_692_localSearch": 
				if(nac_checker.deregister(match)) {
					HMatch acMatch_0 = new LocalSearchMatch("InterfaceConstructorToConstructor__TRG_695_localSearch", 2);
					Object[] acMatch_nodes_0 = acMatch_0.getNodes();
	...
	protected void addReference(ReferenceAdded msg) {
		...
		case "uml.Interface_ownedOperation_Operation": 
			if(!lazy_initialization) {
				{
					HMatch match = new LocalSearchMatch("InterfaceConstructorToConstructor__TRG_695_localSearch", 2);
					Object[] objs = match.getNodes();
					objs[0] = msg.source;
					objs[1] = msg.target;
					currentDepth++;
