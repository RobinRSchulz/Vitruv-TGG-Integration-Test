...
public class FieldStaticToStatic__FWD_643_localSearch extends GenericLocalSearchActor{
	...
	protected void initializePorts(Map<String, ActorRef> name2actor) {
		...
		ports.add(new PortJunction(node.getPorts().getPort().get(0), getSelf(), name2actor.get("FieldStaticToStatic__CONSISTENCY_646_localSearch"), this::returnTrue  , false ));
		ports.add(new PortJunction(node.getPorts().getPort().get(1), getSelf(), name2actor.get("FieldStaticToStatic__FWD_production"), this::returnTrue , 0  , false ));
	}
			...
			case "JavaAttributeToUmlProperty_object_SP3": 
				{
					...
					HMatch match_0 = new LocalSearchMatch("FieldStaticToStatic__FWD_643_localSearch", 4);
			...
			case "Property_object_SP0": 
				{
					if(!lazy_initialization) {
						...
						// umlProperty
						HMatch match_2 = new LocalSearchMatch("FieldStaticToStatic__FWD_643_localSearch", 4);
						match_2.getNodes()[2] = objs[0];
						start(edge_explorer_3_1_orchestration, match_2);
					}
				}
				break;
			case "FieldStaticToStatic__SRC_640_localSearch": 
				{
					if(pac_explorer.register(match)) {
						HMatch acMatch_0 = new LocalSearchMatch("FieldStaticToStatic__FWD_643_localSearch", 4);
		...
		case "Java2Uml.JavaAttributeToUmlProperty_source_Field": 
			if(!lazy_initialization) {
				if(msg.target instanceof org.emftext.language.java.members.Field) {
				{
					...
					HMatch match = new LocalSearchMatch("FieldStaticToStatic__FWD_643_localSearch", 4);
					Object[] objs = match.getNodes();
					objs[0] = msg.source;
					objs[1] = msg.target;
					currentDepth++;
	...
	protected void registerMatch(HMatch match) {
		...
		Object[] objs = match.getNodes();
		// register FieldStaticToStatic__SRC
	...
	protected void deregisterMatch(HMatch match) {
		...
		Object[] objs = match.getNodes();
		// deregister FieldStaticToStatic__SRC
		OverlapMatch oMatch_0 = new OverlapMatch(2);
		Object[] oMatch_Nodes_0 = oMatch_0.getNodes();
		oMatch_Nodes_0[0] = objs[1];
		oMatch_Nodes_0[1] = objs[3];
	...
	public boolean constraint_checker_method(HMatch match) {
		...
		csp_68.getVariables().add(new org.emoflon.ibex.tgg.operational.csp.RuntimeTGGAttributeConstraintVariable(true, umlProperty.isIsStatic(), "boolean"));
		csp_68.solve();
