...
public class MethodStaticToStatic__FWD_1122_localSearch extends GenericLocalSearchActor{
	...
	protected void initializePorts(Map<String, ActorRef> name2actor) {
		...
		ports.add(new PortJunction(node.getPorts().getPort().get(0), getSelf(), name2actor.get("MethodStaticToStatic__CONSISTENCY_1125_localSearch"), this::returnTrue  , false ));
		ports.add(new PortJunction(node.getPorts().getPort().get(1), getSelf(), name2actor.get("MethodStaticToStatic__FWD_production"), this::returnTrue , 0  , false ));
	}
			...
			case "JavaMethodToOperation_object_SP1": 
				{
					...
					HMatch match_1 = new LocalSearchMatch("MethodStaticToStatic__FWD_1122_localSearch", 4);
			...
			case "Operation_object_SP14": 
				{
					if(!lazy_initialization) {
						...
						// umlOperation
						HMatch match_2 = new LocalSearchMatch("MethodStaticToStatic__FWD_1122_localSearch", 4);
						match_2.getNodes()[2] = objs[0];
						start(edge_explorer_3_1_orchestration, match_2);
					}
				}
				break;
			case "MethodStaticToStatic__SRC_1119_localSearch": 
				{
					if(pac_explorer.register(match)) {
						HMatch acMatch_0 = new LocalSearchMatch("MethodStaticToStatic__FWD_1122_localSearch", 4);
		...
		case "Java2Uml.JavaMethodToOperation_source_Method": 
			if(!lazy_initialization) {
				if(msg.target instanceof org.emftext.language.java.members.ClassMethod) {
				{
					...
					HMatch match = new LocalSearchMatch("MethodStaticToStatic__FWD_1122_localSearch", 4);
					Object[] objs = match.getNodes();
					objs[1] = msg.source;
					objs[0] = msg.target;
					currentDepth++;
	...
	protected void registerMatch(HMatch match) {
		...
		Object[] objs = match.getNodes();
		// register MethodStaticToStatic__SRC
	...
	protected void deregisterMatch(HMatch match) {
		...
		Object[] objs = match.getNodes();
		// deregister MethodStaticToStatic__SRC
		OverlapMatch oMatch_0 = new OverlapMatch(2);
		Object[] oMatch_Nodes_0 = oMatch_0.getNodes();
		oMatch_Nodes_0[0] = objs[0];
		oMatch_Nodes_0[1] = objs[3];
	...
	public boolean constraint_checker_method(HMatch match) {
		...
		csp_183.getVariables().add(new org.emoflon.ibex.tgg.operational.csp.RuntimeTGGAttributeConstraintVariable(true, true, "java.lang.String"));
		csp_183.getVariables().add(new org.emoflon.ibex.tgg.operational.csp.RuntimeTGGAttributeConstraintVariable(true, umlOperation.isIsStatic(), "boolean"));
		csp_183.solve();
