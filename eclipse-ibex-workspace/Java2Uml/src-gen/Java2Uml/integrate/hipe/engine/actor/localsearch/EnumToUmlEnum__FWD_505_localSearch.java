...
public class EnumToUmlEnum__FWD_505_localSearch extends GenericLocalSearchActor{
	...
	protected void initializePorts(Map<String, ActorRef> name2actor) {
		...
		ports.add(new PortJunction(node.getPorts().getPort().get(0), getSelf(), name2actor.get("EnumToUmlEnum__CONSISTENCY_508_localSearch"), this::returnTrue  , false ));
		ports.add(new PortJunction(node.getPorts().getPort().get(1), getSelf(), name2actor.get("EnumToUmlEnum__FWD_production"), this::returnTrue  , false ));
	}
			...
			case "JavaPackageToUmlPackage_object_SP2": 
				{
					...
					// javaPackToUmlPack
					HMatch match_1 = new LocalSearchMatch("EnumToUmlEnum__FWD_505_localSearch", 5);
			...
			case "Package_1_object_SP2": 
				{
					if(!lazy_initialization) {
						...
						// umlPackage
						HMatch match_3 = new LocalSearchMatch("EnumToUmlEnum__FWD_505_localSearch", 5);
						match_3.getNodes()[3] = objs[0];
						start(edge_explorer_2_orchestration, match_3);
					}
				}
				break;
			case "EnumToUmlEnum__SRC_501_localSearch": 
				{
					if(pac_explorer.register(match)) {
						HMatch acMatch_0 = new LocalSearchMatch("EnumToUmlEnum__FWD_505_localSearch", 5);
		...
		case "Java2Uml.JavaPackageToUmlPackage_target_Package": 
			if(!lazy_initialization) {
				if(msg.target instanceof uml.Package) {
				{
					...
					HMatch match = new LocalSearchMatch("EnumToUmlEnum__FWD_505_localSearch", 5);
					Object[] objs = match.getNodes();
					objs[1] = msg.source;
					objs[3] = msg.target;
					currentDepth++;
