...
public class MethodAbstractToAbstract__FWD_895_localSearch extends GenericLocalSearchActor{
	...
	protected void initializePorts(Map<String, ActorRef> name2actor) {
		...
		ports.add(new PortJunction(node.getPorts().getPort().get(0), getSelf(), name2actor.get("MethodAbstractToAbstract__CONSISTENCY_898_localSearch"), this::returnTrue  , false ));
		ports.add(new PortJunction(node.getPorts().getPort().get(1), getSelf(), name2actor.get("MethodAbstractToAbstract__FWD_production"), this::returnTrue , 0  , false ));
	}
			...
			case "JavaMethodToOperation_object_SP3": 
				{
					...
					HMatch match_1 = new LocalSearchMatch("MethodAbstractToAbstract__FWD_895_localSearch", 4);
			...
			case "Operation_object_SP7": 
				{
					if(!lazy_initialization) {
						...
						// umlOperation
						HMatch match_2 = new LocalSearchMatch("MethodAbstractToAbstract__FWD_895_localSearch", 4);
						match_2.getNodes()[2] = objs[0];
						start(edge_explorer_3_1_orchestration, match_2);
					}
				}
				break;
			case "MethodAbstractToAbstract__SRC_892_localSearch": 
				{
					if(pac_explorer.register(match)) {
						HMatch acMatch_0 = new LocalSearchMatch("MethodAbstractToAbstract__FWD_895_localSearch", 4);
		...
		case "Java2Uml.JavaMethodToOperation_source_Method": 
			if(!lazy_initialization) {
				if(msg.target instanceof org.emftext.language.java.members.ClassMethod) {
				{
					...
					HMatch match = new LocalSearchMatch("MethodAbstractToAbstract__FWD_895_localSearch", 4);
					Object[] objs = match.getNodes();
					objs[1] = msg.source;
					objs[0] = msg.target;
					currentDepth++;
	...
	protected void registerMatch(HMatch match) {
		...
		Object[] objs = match.getNodes();
		// register MethodAbstractToAbstract__SRC
	...
	protected void deregisterMatch(HMatch match) {
		...
		Object[] objs = match.getNodes();
		// deregister MethodAbstractToAbstract__SRC
		OverlapMatch oMatch_0 = new OverlapMatch(2);
		Object[] oMatch_Nodes_0 = oMatch_0.getNodes();
		oMatch_Nodes_0[0] = objs[0];
		oMatch_Nodes_0[1] = objs[3];
	...
	public boolean constraint_checker_method(HMatch match) {
		...
		csp_143.getVariables().add(new org.emoflon.ibex.tgg.operational.csp.RuntimeTGGAttributeConstraintVariable(true, true, "java.lang.String"));
		csp_143.getVariables().add(new org.emoflon.ibex.tgg.operational.csp.RuntimeTGGAttributeConstraintVariable(true, umlOperation.isIsAbstract(), "boolean"));
		csp_143.solve();
