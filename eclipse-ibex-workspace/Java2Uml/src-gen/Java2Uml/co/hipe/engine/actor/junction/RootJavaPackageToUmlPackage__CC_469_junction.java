...
public class RootJavaPackageToUmlPackage__CC_469_junction extends GenericJunctionActor{
	...
	protected void registerMatchForAttributeChanges(HMatch match) {
		...
		Collection<HMatch> namedElement_0_Matches = namedElement_42_AttrMap.get(matchObjects[0]);
		if(namedElement_0_Matches == null) {
			namedElement_0_Matches = new LinkedList<>();
			namedElement_42_AttrMap.put(matchObjects[0], namedElement_0_Matches);
		}
		
		namedElement_0_Matches.add(match);
		
		Collection<HMatch> namedElement_1_Matches = namedElement_43_AttrMap.get(matchObjects[1]);
		if(namedElement_1_Matches == null) {
			namedElement_1_Matches = new LinkedList<>();
			namedElement_43_AttrMap.put(matchObjects[1], namedElement_1_Matches);
		}
	...
	protected void deregisterMatchForAttributeChanges(Set<HMatch> matches, HMatch match) {
		Object[] matchObjects = match.getNodes();
		Collection<HMatch> matches_0 = namedElement_42_AttrMap.get(matchObjects[0]);
		if(matches_0 != null) {
			matches_0.remove(match);
		}
		Collection<HMatch> matches_1 = namedElement_43_AttrMap.get(matchObjects[1]);
		if(matches_1 != null) {
